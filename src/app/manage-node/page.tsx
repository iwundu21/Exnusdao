  ...
  const handleWithdrawSeed = (vId: string) => {
    ...
    setState(prev => ({
      ...
      validators: prev.validators.map(v => v.id === vId ? { 
        ...
        is_active: false,
        total_staked: Math.max(0, v.total_staked - SEED_DEPOSIT_AMOUNT) 
      } : v)
    }));
                      ...
                      {!node.seed_deposited ? (
                        ...
                        <div className="space-y-4">
                          ...
                          <button 
                            ...
                            className="w-full py-2 border border-red-500/20 text-red-400 text-[10px] font-black uppercase rounded-lg hover:bg-red-500/10 transition-all flex items-center justify-center gap-2"
                          >
