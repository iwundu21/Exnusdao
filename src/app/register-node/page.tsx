...
export default function RegisterNodePage() {
    ...
    return (
      <div className="flex flex-col items-center justify-center text-center px-10 py-32 space-y-6 animate-in fade-in duration-500">
         ...
         <div className="space-y-3">
           <h1 className="text-2xl font-black uppercase tracking-tighter text-foreground">AUTH_REQUIRED</h1>
           <p className="text-muted-foreground text-[10px] uppercase font-black tracking-[0.3em]">Please connect your Solana wallet to verify XNode License ownership.</p>
         </div>
      </div>
    );
  }
  ...
  return (
    <div className="max-w-6xl mx-auto px-10 py-16 space-y-10 animate-in fade-in duration-500">
      ...
      <div className="space-y-3">
        <h1 className="text-4xl font-black exn-gradient-text tracking-tighter uppercase text-foreground leading-none">XNODE_PROVISIONING</h1>
      ...
      {hasExistingNode ? (
        <div className="exn-card p-12 flex flex-col items-center justify-center text-center space-y-8 border-amber-500/30 bg-amber-500/5 shadow-2xl backdrop-blur-3xl">
           <AlertCircle className="w-12 h-12 text-amber-500" />
           <div className="space-y-3">
             <h2 className="text-2xl font-black text-foreground uppercase tracking-tighter">ACTIVE_XNODE_DETECTED</h2>
             <p className="text-[10px] text-muted-foreground uppercase font-black tracking-[0.3em]">Only one XNode registration permitted per unique wallet address.</p>
           </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
          <div className="lg:col-span-7 exn-card p-8 space-y-10 bg-black/40 backdrop-blur-3xl border-white/10 shadow-3xl">
            <div className="space-y-6">
              <div className="flex items-center justify-between border-b border-white/5 pb-4">
                 <h3 className="text-[9px] font-black text-white/40 uppercase tracking-[0.4em]">NFT_AUTHORIZATION</h3>
              </div>
              
              <div className="space-y-3">
                <label className="text-[9px] text-white/30 uppercase font-black tracking-[0.3em]">SELECT_XNODE_LICENSE *</label>
                <div className="relative group">
                   <Ticket className="absolute left-4 top-3.5 w-4.5 h-4.5 text-primary/40 group-hover:text-primary transition-colors" />
                   <select 
                    value={formData.licenseId} 
                    onChange={e => setFormData({...formData, licenseId: e.target.value})} 
                    className="exn-input h-12 pl-12 text-[10px] font-mono font-black tracking-tight"
                   >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="space-y-3">
                <label className="text-[9px] text-white/30 uppercase font-black tracking-[0.3em]">XNODE_NAME *</label>
                <input value={formData.name} onChange={e => setFormData({...formData, name: e.target.value})} className="exn-input h-12 text-xs font-mono font-bold" placeholder="ID_STRING_V1" />
              </div>
              <div className="space-y-3">
                <label className="text-[9px] text-white/30 uppercase font-black tracking-[0.3em]">SECTOR_LOCATION *</label>
                <input value={formData.location} onChange={e => setFormData({...formData, location: e.target.value})} className="exn-input h-12 text-xs font-mono font-bold" placeholder="e.g. FRANKFURT_DE" />
              </div>
              
              <div className="md:col-span-2 space-y-3">
                <label className="text-[9px] text-white/30 uppercase font-black tracking-[0.3em]">IDENTITY_ASSET (LOGO_UPLOAD)</label>
                <div 
                  onClick={() => fileInputRef.current?.click()}
                  className="w-full h-32 border-2 border-dashed border-white/10 rounded-xl flex flex-col items-center justify-center gap-3 hover:border-primary/40 hover:bg-primary/5 transition-all cursor-pointer group shadow-inner"
                >
                  <Upload className="w-6 h-6 text-white/20 group-hover:text-primary transition-all duration-500" />
                  <p className="text-[9px] uppercase font-black text-white/30 group-hover:text-primary transition-all tracking-[0.2em]">
              ...
              <div className="space-y-3">
                <label className="text-[9px] text-white/30 uppercase font-black tracking-[0.3em]">COMMISSION_TIER (0-30%)</label>
                <div className="relative">
                  <input type="number" value={formData.commission} onChange={e => setFormData({...formData, commission: Math.min(30, Math.max(0, Number(e.target.value)))})} className="exn-input h-12 text-xs font-mono font-bold" />
                  <span className="absolute right-5 top-3.5 text-white/40 font-black text-[9px]">%</span>
                </div>
              </div>

              <div className="md:col-span-2 space-y-3">
                <label className="text-[9px] text-white/30 uppercase font-black tracking-[0.3em]">PROTOCOL_BIO *</label>
                <textarea value={formData.description} onChange={e => setFormData({...formData, description: e.target.value})} className="exn-input min-h-[100px] py-4 text-[11px] font-mono font-medium leading-relaxed" placeholder="Describe hardware commitment..." />
              </div>
            </div>
            ...
            <button 
              ...
              className={`w-full h-14 uppercase tracking-[0.5em] font-black transition-all shadow-3xl text-[11px] ${isRegistrationDisabled ? 'bg-white/5 text-white/10 border border-white/10 cursor-not-allowed opacity-50' : 'exn-button'}`}
            >
              REVIEW_PROVISIONING
            </button>
          </div>
          ...
          <div className="lg:col-span-5 space-y-8">
            <div className="sticky top-32">
              <h3 className="text-[9px] font-black text-white/40 uppercase tracking-[0.4em] mb-5">IDENTITY_PREVIEW_V2</h3>
              <div className="exn-card overflow-hidden border-primary/30 bg-black/60 shadow-[0_40px_100px_rgba(0,0,0,0.8)] backdrop-blur-3xl group">
                <div className="relative h-48 w-full overflow-hidden">
                  ...
                  <div className="absolute top-4 right-6">
                    <div className="flex items-center gap-1.5 bg-emerald-500 text-black text-[9px] px-3 py-1.5 rounded-lg font-black uppercase border border-emerald-400 shadow-[0_0_30px_rgba(16,185,129,0.5)] animate-in fade-in zoom-in duration-700">
                ...
                <div className="p-8 space-y-8">
                  <div className="space-y-3">
                    <p className="text-[9px] text-white/20 uppercase font-black tracking-[0.4em]">NETWORK_BIO</p>
                    <p className="text-[11px] text-white/60 leading-relaxed font-medium italic min-h-[3rem] tracking-tight">
                      {formData.description || "Establish hardware metrics..."}
                    </p>
                  </div>

                  <div className="grid grid-cols-2 gap-4">
                    <div className="p-5 bg-white/5 rounded-xl border border-white/10 space-y-1.5 group-hover:border-primary/40 transition-all shadow-xl">
                      <p className="text-[9px] uppercase font-black text-white/20 tracking-widest">NODE_FEE</p>
                      <p className="text-primary font-black text-[12px] font-mono tracking-tighter">{(formData.commission).toFixed(1)}%</p>
                    </div>
                    <div className="p-5 bg-white/5 rounded-xl border border-white/10 space-y-1.5 group-hover:border-primary/40 transition-all shadow-xl">
                      <p className="text-[9px] uppercase font-black text-white/20 tracking-widest">NFT_BOND</p>
                      <p className="text-foreground font-mono font-black text-[9px] truncate tracking-tighter">{formData.licenseId ? shortenAddress(formData.licenseId) : 'N/A'}</p>
                    </div>
                  </div>
                </div>
              </div>
        ...
        <AlertDialogContent className="exn-card border-primary/50 bg-black/95 backdrop-blur-3xl p-0 overflow-hidden max-w-md">
          <div className="p-8 space-y-8">
            ...
            <AlertDialogFooter className="flex flex-row gap-4 pt-2">
              ...
              <AlertDialogAction onClick={confirmRegistration} className="exn-button flex-1 text-[10px] h-12 uppercase font-black">CONFIRM_BIND</AlertDialogAction>
            </AlertDialogFooter>
          </div>
        </AlertDialogContent>
      </AlertDialog>
