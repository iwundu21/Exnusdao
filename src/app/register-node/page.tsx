...
export default function RegisterNodePage() {
  ...
  // Check if user already owns a node
  ...
  const handleRegister = () => {
    if (hasExistingNode) {
      return toast({ 
        ...
        description: "Protocol limits strictly enforce one node per wallet identity.", 
        variant: "destructive" 
      });
    }
      ...
      <div className="max-w-4xl mx-auto px-10 py-20 space-y-12">
        ...
        <div className="space-y-4">
          ...
          <p className="text-white/40 max-w-xl">
            Register your high-performance node. Each registration requires a valid protocol license and is limited to one per user.
          </p>
        </div>
        ...
        {hasExistingNode ? (
          <div className="exn-card p-12 flex flex-col items-center justify-center text-center space-y-8 border-amber-500/20 bg-amber-500/5">
             ...
             <div className="space-y-2">
                <h2 className="text-2xl font-bold text-white uppercase tracking-tight">Active Node Detected</h2>
                <p className="text-white/40 max-w-md text-sm">
                   Your wallet <span className="text-amber-500 font-mono">{USER_WALLET}</span> is already associated with the node <span className="text-white font-bold">"{existingNode.name}"</span>. 
                   The Exnus protocol enforces a 1-to-1 relationship between wallets and validator nodes.
                </p>
             </div>
             <Link href="/manage-node" className="exn-button flex items-center gap-2">
                Manage Existing Node <ExternalLink className="w-4 h-4" />
             </Link>
          </div>
        ) : (
